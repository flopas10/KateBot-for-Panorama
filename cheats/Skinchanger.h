	...
	try {
		while (true)
		{
			...
			for (int i = 1; i < 10; i++) {
				...
				int WeaponID = mem->Read<short>(WeaponBase + ofs->m_iItemDefinitionIndex);
				if (WeaponID < 65) {
...
[junk_enable /]
					...
					if (KnifeBase >= 1000) {
						...
						if (mem->Read<short>(WeaponBase + ofs->m_iItemDefinitionIndex) != GetKnifeID(cfg->knifeModel)) {
							mem->Write<short>(WeaponBase + ofs->m_iItemDefinitionIndex, GetKnifeID(cfg->knifeModel));
						}
					}
				}
			}
